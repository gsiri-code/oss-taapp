...
- `delete_tasklist(tasklist_id: str) -> bool`: Remove a task list by its ID.

#### Task Operations

- `list_tasks(tasklist_id: str) -> list[Task]`: Return all tasks in a task list.
- `get_task(tasklist_id: str, task_id: str) -> Task`: Return a single task by ID.
- `insert_task(tasklist_id: str, task: Task) -> Task`: Create a new task in a tasklist.
- `delete_task(tasklist_id: str, task_id: str) -> bool`: Remove a task by its ID.
...
- `get_task(raw_data: str) -> Task`: Returns a Task instance from raw data.
- `get_tasklist(raw_data: str) -> TaskList`: Returns a TaskList instance from raw data.
...
```python
from task_client_api import get_client
from task_client_api.task import get_task
from task_client_api.tasklist import TaskList
import json
...
# Create a new task
task_data = json.dumps({"title": "My new task"})
task = get_task(task_data)
new_task = client.insert_task(tasklist.id, task)
client.delete_task(tasklist.id, new_task.id)
```
