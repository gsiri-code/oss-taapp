...
def service_base_url() -> Generator[str, None, None]:
    ...
    USING_STUB_SERVICE = bool(missing_env_vars)

    if not USING_STUB_SERVICE:
...
def test_env_override_allows_env_credentials_in_fastapi_context(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(OAuthManager, "_is_in_fastapi_context", lambda self: True)
    monkeypatch.setattr(OAuthManager, "_get_session_credentials", lambda self: None)
